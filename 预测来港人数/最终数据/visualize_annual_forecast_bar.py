...
colors = {
    ...
    'transition': '#A8A8A8'   # Gray for transition
}
...
# Plot historical data bars
bar_width = 0.8
historical_bars = ax.bar(annual_historical['year'], 
                        annual_historical['total arrivals'] / 1e6,
                        bar_width,
                        color=colors['historical'],
                        label='Historical Data (2013-2023)')

# Create transition bar for 2024
...
transition_value = last_historical_value * 0.4 + first_forecast_value * 0.6

ax.bar(2024, transition_value, bar_width,
       color=colors['transition'],
       alpha=0.5,
       hatch='///',
       label='Transition Period (2024)')

# Plot forecast data bars
forecast_bars = ax.bar(annual_forecast['year'],
                      annual_forecast['predicted'] / 1e6,
                      bar_width,
                      color=colors['forecast'],
                      label='Forecast (2025-2030)')

# Add error bars for forecast data
ax.errorbar(annual_forecast['year'],
           annual_forecast['predicted'] / 1e6,
           yerr=[(annual_forecast['predicted'] - annual_forecast_ci['lower_ci']) / 1e6,
                 (annual_forecast_ci['upper_ci'] - annual_forecast['predicted']) / 1e6],
           fmt='none',
           color='black',
           capsize=5,
           capthick=1,
...
ax.text(mid_year, reference_value + 1, f'Pre-pandemic Average: {reference_value:.1f}M',
        va='bottom', ha='center', color=colors['reference'],
...
add_value_labels(historical_bars)
add_value_labels(forecast_bars)

# Add transition period value
ax.text(2024, transition_value, f'{transition_value:.1f}M',
        ha='center', va='bottom', fontsize=9)
...
plt.savefig('annual_forecast_visualization_bar.png', dpi=300, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.close()

print("\nBar chart visualization has been generated: annual_forecast_visualization_bar.png") 